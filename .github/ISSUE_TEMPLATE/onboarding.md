---
name: Onboarding
about: Use this template to onboard new team members to GC Forms
title: ''
labels: ''
assignees: ''

---

## Product onboarding checklist

- [ ] Overview of the product context including vision, roadmap, and value proposition:
    - [ ] [Product Overview](https://docs.google.com/document/d/13XU8m-7Az2MHil-bqAimIAJMYSkiJzBUZGnJEXU2RS0/edit?tab=t.0#heading=h.yp9lldruzgrx)
    - [ ] [GC Forms Context and History](https://docs.google.com/presentation/d/1ygRMbyhGjuwCRv6_C8xEypSi0zuzAXjBgIUzz_DX498/edit?slide=id.g4e9b57e4d8_0_178#slide=id.g4e9b57e4d8_0_178)
    - [ ] [Exemplar Forms](https://docs.google.com/document/d/1LpBehnBWWBjfoJX9-ruttwdD1M0_Ul2QV2nN5nr4H3g/edit?tab=t.0#heading=h.auwtug3rp529)
    - [ ] [Strategic Roadmap](https://articles.alpha.canada.ca/forms-formulaires/roadmap/)
    - [ ] [GC Forms OKRs for 2026-27](https://docs.google.com/document/d/16m0RQYR6An32kVzIq44MERcfP3U_WxeryLRMXFEPuDw/edit?tab=t.0)
    - [ ] [Sprint Goals](https://docs.google.com/document/d/1HxnnPBMZKNL8pmI_Ylz7Bl35teFbJZ6S0rtHmD31N1A/edit?tab=t.0#heading=h.6m6rslpk8tyy)
- [ ] Review objectives and roadmap with product manager
- [ ] Review policy and government 101
- [ ] Review key research artifacts
- [ ] Review ways of working with delivery manager

### Developer specific:

- [ ] ZenHub for issue management and GitHub integration
- [ ] LastPass Shared folder (for secrets)
- [ ] _Ways of working_
  - [ ] Branching Strategy
  - [ ] Sprint Poker
  - [ ] Github tips and tricks [Github Docs](https://docs.github.com/en/issues/tracking-your-work-with-issues/linking-a-pull-request-to-an-issue#linking-a-pull-request-to-an-issue-using-a-keyword)
- [ ] _Technical onboarding_
  - [ ] [Forms Solution Architecture](https://drive.google.com/file/d/1Fs6McxOOlJJPykKm-FosHwHyfNjpOnaG/view?usp=sharing)
  - [ ] [NextJS](https://nextjs.org/)
  - [ ] [Next-i18next](https://github.com/isaachinman/next-i18next)
  - [ ] [Formik](https://formik.org/docs/overview)
  - [ ] [Next-Auth](https://next-auth.js.org/)
  - [ ] [platform-forms-client](https://github.com/cds-snc/platform-forms-client) repository walkthrough to explain structure and data flow within NextJS app.
  - [ ] Lambda and Reliability Queue walkthrough to explain data flow
  - [ ] Vault and Notify processing walkthrough to explain processing (rehydration of responses, file attachments, etc)

## Tooling access
- [ ] Set-up access to Github, Google Drive, Figma, and other required accounts

## GC Forms itself
_Feel free to create accounts, explore docs and play around in the app_
- [ ] [Staging](https://forms-staging.cdssandbox.xyz/ )
- [ ] [Production](https://forms-formulaires.alpha.canada.ca/en/auth/login)
- [ ] [SSO portal](https://auth.cdssandbox.xyz/ui/v2) 
- [ ] [Marketing site](https://articles.alpha.canada.ca/forms-formulaires/)
- [ ] [API documentation](https://cds-snc.github.io/forms-api/)

### Slack channels
_Main channels (we also have some temporary slack channels for features in flight)_
- [ ] [#forms-shared](https://app.slack.com/client/T2G2S06PM/C0117FN8G9K)
- [ ] [#forms-dev](https://app.slack.com/client/T2G2S06PM/C01GFD150MC)

### Github repos
_Main repos_
- [ ] [platform-forms-client](https://github.com/cds-snc/platform-forms-client)
- [ ] [forms-terraform](https://github.com/cds-snc/forms-terraform)
- [ ] [forms-api](https://github.com/cds-snc/forms-api)
- [ ] [platform-unified-accounts-user-portal](https://github.com/cds-snc/platform-unified-accounts-user-portal)

### AWS accounts
_For developers_
- [ ] Staging
- [ ] Production (IF NEEDED ONLY)

### Other
- [ ] [Google Drive (Forms folder)](https://drive.google.com/drive/folders/1owWQrfuFLyQh0iHYEZT_irDLaTB9xpEN)
- [ ] [Figma](https://www.figma.com/files/1053802535417200651/project/11395874?fuid=761305766059523500)
- [ ] [Miro](https://miro.com/app/dashboard/space/3IoJPdPICWiaPZxVkgvrMU)
- [ ] [ZenHub](https://app.zenhub.com/workspaces/gcforms-60cb8929764d71000e481cab/board)
- [ ] Trello (if needed)
- [ ] GC Articles (if needed)

## Connect with team members

- [ ] Head of (Director) GC Forms
- [ ] Product Manager / Delivery Manager
- [ ] Tech lead(s)
- [ ] Designer(s) / Content designer
- [ ] Developers
- [ ] Researcher
- [ ] Policy advisor
- [ ] Growth / CX / Support (if applicable)

## Identify good onboarding tasks/issues to build familiarity

- 

