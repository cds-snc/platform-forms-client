...
export const updatePublishing = AuthenticatedAction(
  async (userID: string, publishFormsId: string, action: "add" | "remove") => {
...
export const updateActive = AuthenticatedAction(async (userID: string, active: boolean) => {
  await updateActiveStatus(userID, active);
  revalidatePath("(gcforms)/[locale]/(app administration)/admin/(with nav)/accounts", "page");
});

export const getAllUsers = AuthenticatedAction(async (active?: boolean) => {
