...
describe("processResponse with null values", () => {
  let dir: InMemoryDirectoryHandle;
  beforeEach(() => {
    // Re-create prepared environment using fixtures with null values
    preparedEnv = prepareTestEnvFromFixtures(
      submissionFixtureWithNull as unknown as SubmissionFixture,
      templateFixtureWithNull as unknown
    );
    dir = preparedEnv.dir;
  });

  it("writes a CSV row from a submission with null values", async () => {
    const env = preparedEnv;
    const formTemplate = env.formTemplate as unknown as FormProperties;
    const formId = "cmeaj61dl0001xf01aja6mnpf";
    const responseName = "26-11-2b09f";
    ...
    // Ensure known fields from submission are present in CSV
    expect(text).toContain("Qui sunt est elit culpa."); // Name from field 1
    expect(text).toContain("proident@laborum.ca"); // Email from field 5
    expect(text).toContain("test.docx"); // Attachment filename
  });
});
