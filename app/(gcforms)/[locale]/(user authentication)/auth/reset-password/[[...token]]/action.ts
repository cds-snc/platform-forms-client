...
const validateQuestionChallengeForm = async (
  ...
  const schema = v.object({
    ...
    question1Id: v.pipe(v.string(), v.minLength(1, t("input-validation.required"))),
    question2: v.pipe(
      v.string(),
      v.toLowerCase(),
      v.trim(),
    ...
    question2Id: v.pipe(v.string(), v.minLength(1, t("input-validation.required"))),
    question3: v.pipe(
      v.string(),
      v.toLowerCase(),
      v.trim(),
    ...
    question3Id: v.pipe(v.string(), v.minLength(1, t("input-validation.required"))),
    email: v.pipe(
      v.string(),
      v.toLowerCase(),
      v.trim(),
...
const validatePasswordResetForm = async (
  ...
  const schema = v.pipe(
    v.object({
      confirmationCode: v.pipe(v.unknown(), v.transform(Number)),
      username: v.pipe(
        v.string(),
        v.toLowerCase(),
        v.trim(),
