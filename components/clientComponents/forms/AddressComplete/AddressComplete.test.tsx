    ...
    (props, ref) => {
      ...
      return (
        <div data-testid={testId}>
          ...
          <button
            type="button"
            data-testid={`${props.id}-set`}
            onClick={() => props.onSetValue?.(inputValue)}
          >
            set
          </button>
          <button
...
describe("AddressComplete", () => {
  ...
  it("uses AddressComplete search when feature flag is enabled", async () => {
    ...
    await waitFor(() => {
      expect(getAddressCompleteChoicesMock).toHaveBeenLastCalledWith("123 Main", "CAN", "en", "form-123", "");
    });
  ...
  it("retrieves and applies selected full address for Retrieve results", async () => {
    ...
    await waitFor(() => {
      expect(getSelectedAddressMock).toHaveBeenCalledWith("id-retrieve-1", "CAN", "en", "form-123", "");
    });
  ...
  it("does nested lookup when selected result next is Find", async () => {
    ...
    await waitFor(() => {
      expect(getAddressCompleteRetrieveMock).toHaveBeenCalledWith("nested-id-1", "CAN", "en", "form-123", "");
    });
  ...
  it("resets address fields when country changes and uses selected country for lookup", async () => {
    ...
    await waitFor(() => {
      expect(getAddressCompleteChoicesMock).toHaveBeenLastCalledWith("10 Rue", "FRA", "en", "form-123", "");
    });
  });
  ...
  it("localizes nested address descriptions in French", async () => {
    ...
    await waitFor(() => {
      expect(getAddressCompleteChoicesMock).toHaveBeenLastCalledWith("222 King", "CAN", "fr", "form-123", "");
    });
