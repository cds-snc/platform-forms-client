...
/*  As per the spec, ComboBox includes a HTML <select> with options AND a separate <input> and dropdown <ul> with items.
    The select is gc-sr-only and is always hidden via CSS. The input and dropdown list are the elements used for interaction.
...
export const ComboBox = (props: ComboBoxProps): React.ReactElement => {
  ...
  const containerClasses = classnames('gc-combo-box', className, {
    'gc-combo-box--pristine': state.selectedOption,
  })
  ...
  return (
    <div
      ...
      ref={containerRef}>
      <select
        className="gc-select gc-sr-only gc-combo-box__select"
        name={name}
        aria-hidden
        tabIndex={-1}
      ...
      <Input
        ...
        {...inputProps}
      />
      <span className="gc-combo-box__clear-input__wrapper" tabIndex={-1}>
        <button
          type="button"
          className="gc-combo-box__clear-input"
      ...
      <span className="gc-combo-box__input-button-separator">&nbsp;</span>
      <span className="gc-combo-box__toggle-list__wrapper" tabIndex={-1}>
        <button
          data-testid="combo-box-toggle"
          type="button"
          className="gc-combo-box__toggle-list"
          tabIndex={-1}
      ...
      <ul
        ...
        tabIndex={-1}
        id={listID}
        className="gc-combo-box__list"
        role="listbox"
        hidden={!state.isOpen}>
        {state.filteredOptions.map((option, index) => {
          const focused = option === state.focusedOption
          const selected = option === state.selectedOption
          const itemClasses = classnames('gc-combo-box__list-option', {
            'gc-combo-box__list-option--focused': focused,
            'gc-combo-box__list-option--selected': selected,
          })
        ...
        {state.filteredOptions.length === 0 ? (
          <li className="gc-combo-box__list-option--no-results">
            {noResults || 'No results found'}
          </li>
        ) : null}
      </ul>

      <div className="gc-combo-box__status gc-sr-only" role="status"></div>
      <span
        id={assistiveHintID}
        className="gc-sr-only"
