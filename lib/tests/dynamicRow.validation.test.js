...
describe("Dynamic rows validator", () => {
  test("Parses form values data", () => {
    expect(mockDynaRowFormRes.length === 3);
  });
});

describe("extractSubElementId: dynamic rows sub's ids validator", () => {
  test("Should extract element id ", () => {
    const elem = dynForm.formConfig.elements.find((element) => element.id === 3);
    expect(
      extractSubElementId(elem.properties.subElements.find((subElem) => subElem.id === 4)) === "0"
    );
  });
