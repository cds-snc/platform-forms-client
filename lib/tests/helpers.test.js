...
describe("Submit and Template helpers", () => {
  ...
  test("Should submit with the correct csrf token", async () => {
    getCsrfToken.mockReturnValueOnce("csrfTokenSecretValue");
    ...
    expect(mockedAxios.mock.calls[0][0].headers["X-CSRF-Token"]).toBe("csrfTokenSecretValue");
    expect(mockedAxios.mock.calls[0][0].data).toEqual(buildFormDataObject(TestForm, mockResponses));
    expect(mockedRouter.push).toBeCalledTimes(1);
  });

  test("Should fail on submit", async () => {
    jest.spyOn(logMessage, "error");
    mockedAxios.mockResolvedValue({
      status: 400,
    ...
    await submitToAPI(mockResponses, mockedFormikBag);
    expect(mockedAxios.mock.calls[0][0].headers["X-CSRF-Token"]).toBe(undefined);
    expect(mockedFormikBag.setStatus).toBeCalledWith("Error");
    expect(mockedRouter.push).toBeCalledTimes(0);
  });
});
