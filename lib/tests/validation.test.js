...
describe("Test extractSubElementId", () => {
  const testInputData = {
    1: {
      id: 5,
      type: "richText",
  ...
  test("Should return a valid parent/sub element id 1:1", () => {
    expect(extractSubElementId(testInputData[2])).toMatchObject({ parentId: "1", childId: "1" });
  });
  ...
  test("Should thrown an exception invalid id", () => {
    expect(() => extractSubElementId({})).toThrow();
  });

  test("Should return a valid parent and child Ids 0:1", () => {
    expect(extractSubElementId(testInputData[1])).toMatchObject({ parentId: "0", childId: "0" });
  });
});
