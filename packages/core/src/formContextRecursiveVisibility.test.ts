  ...
  test("Visibility with groups", async () => {
    const formRecord = {
      ...
      form: {
        ...
        layout: [1, 3, 4, 5, 12, 7, 8, 9, 10, 11, 13],
        elements: [
          {
            id: 10,
            type: "textArea",
    ...
    expect(checkVisibilityRecursive(formRecord, getElement(13), valuesHasAccountReferYes)).toBe(
      true
    );
    // Should show discount code (11) only if "Do you have an account?" is "yes"
    expect(checkVisibilityRecursive(formRecord, getElement(11), valuesHasAccountReferYes)).toBe(
      true
    );
    // Should not show refer field if refer=no
    const valuesHasAccountReferNo = {
      ...valuesHasAccountReferYes,
      "12": "no",
      matchedIds: ["12.1", "1.0"],
    };
    expect(checkVisibilityRecursive(formRecord, getElement(13), valuesHasAccountReferNo)).toBe(
      false
    );
