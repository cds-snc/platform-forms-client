...
- Soyez prudent lorsque vous ouvrez des fichiers joints à des soumissions de formulaires : vérifiez si la fonctionnalité d'analyse de fichiers de Formulaires GC a détecté un risque potentiel; choisissez un emplacement sécurisé; suivez les consignes de sécurité; et effectuez un test avec un logiciel antivirus local si possible. Vous êtes responsables, avec votre équipe de sécurité, de décider comment vous souhaitez vous protéger contre les fichiers malveillants. La sécurité des fichiers ne peut être garantie, car aucun logiciel n'est parfait pour détecter les menaces.
