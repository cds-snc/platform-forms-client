...
existing_load_balancer_listener_rules=$(
  aws elbv2 describe-rules \
    ...
    --no-paginate \
    --query "Rules[?Actions[0].ForwardConfig.TargetGroups[0].TargetGroupArn && contains(Actions[0].ForwardConfig.TargetGroups[0].TargetGroupArn, '/rainbow-$shorter_deployment_identifier/')].RuleArn" \
    | jq -r ".[]"
)
