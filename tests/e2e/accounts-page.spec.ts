...
test.describe("Accounts Page", () => {
  ...
  test.describe("Accounts tabs/filters and cards", () => {
    test("Clicking tabs/filters updates with expected content", async ({ page }) => {
      ...
      await expect(page.locator("ul[data-testid='accountsList'] li")).toContainText(deactivatedUserEmail);
    ...
    test.skip("Clicking lock/unlock publishing of an account updates the button text state", async ({ page }) => {
      await page.getByRole("button", { name: "All" }).click();
      // Lock an account
      await page.locator(`li[data-testid="${testUserEmail}"]`).getByRole("button", { name: "Lock publishing" }).click();
      await expect(page.locator(`li[data-testid="${testUserEmail}"]`)).toContainText("Unlock publishing");

      // Unlock an account
      await page.locator(`li[data-testid="${testUserEmail}"]`).getByRole("button", { name: "Unlock publishing" }).click();
      await expect(page.locator(`li[data-testid="${testUserEmail}"]`)).toContainText("Lock publishing");
    });

    test("Clicking manage forms navigates to the related page", async ({ page }) => {
      await page.getByRole("button", { name: "All" }).click();
      await page.locator(`li[data-testid="${testUserEmail}"]`).getByRole("button", { name: "Manage forms" }).click();
      await expect(page.getByRole("heading", { level: 1 })).toContainText("Manage forms");
    });

    // Skipping for now --- until we have time to dig in on this further
    test.skip("Account deactivation updates the card and related tabs/filters lists correctly", async ({ page }) => {
      // Deactivate the test.user and check the tab states updated correctly
      await page.getByRole("button", { name: "All" }).click();
      await page.locator(`li[data-testid="${testUserEmail}"]`).getByRole("button", { name: "More" }).click();
      ...
      await expect(page.locator(`li[data-testid="${testUserEmail}"]`)).toContainText("Reactivate account");
      ...
      await page.getByRole("button", { name: "All" }).click();
      await page.locator(`li[data-testid="${testUserEmail}"]`).getByRole("button", { name: "Reactivate account" }).click();
